...
type Health = { ok: boolean; generatedAt: string; checks: Array<{ key: string; label: string; ok: boolean; detail: string }> };
...
export default function WorkshopHealthPanel({ adminKey }: { adminKey: string }) {
  ...
  return <section className="workshop-admin-health">
    ...
    <div>{health?.checks.map((check) => <article key={check.key} className={check.ok ? "is-ok" : "is-pending"}><i>{check.ok ? "✓" : "!"}</i><span><strong>{check.label}</strong><small>{check.detail}</small></span></article>) ?? <p>Verificando base de datos, Storage, grabación y correo…</p>}</div>
  </section>;
}
