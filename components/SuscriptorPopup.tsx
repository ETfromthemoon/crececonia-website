...
function SuscriptorPopupInner() {
  ...
  const forceOpen = searchParams.get("suscribir") === "1";
  const rawSource = searchParams.get("utm_source") ?? "";
  const source = SOCIAL_SOURCES.includes(rawSource) ? rawSource : "suscriptor_link";

  useEffect(() => {
    if (typeof window === "undefined") return;
    if (!forceOpen) return;

    // Limpia el param de la URL sin disparar navegación
    const clean = () => {
      const params = new URLSearchParams(window.location.search);
      params.delete("suscribir");
      const qs = params.toString();
      window.history.replaceState(null, "", window.location.pathname + (qs ? "?" + qs : ""));
    };

    // Ya suscrito: solo limpia URL
          ...
          >
            <div
              ...
              style={{
                ...
                borderRadius: 4,
                boxShadow:
                  "0 0 0 1px rgba(217,179,106,0.05), 0 32px 80px rgba(0,0,0,0.6)",
              }}
            >
              {/* Botón cerrar */}
              <button
                onClick={dismiss}
                className="absolute top-4 right-4 w-7 h-7 flex items-center justify-center text-lg transition-opacity hover:opacity-70"
              ...
              {status === "success" ? (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm" style={{ color: "var(--ash)" }}>
                    Te enviamos lo mejor de IA aplicada al negocio, cada semana.
                  </p>
                </div>
              ) : (
                <>
                  ...
                  >
                    Suscríbete y recibe en tu correo lo mejor de IA aplicada a
                    negocios: herramientas prácticas, guías paso a paso, acceso
                    anticipado a cursos y descuentos exclusivos.
                  </p>
...
export default function SuscriptorPopup() {
  return (
    <Suspense fallback={null}>
      <SuscriptorPopupInner />
    </Suspense>
  );
}
