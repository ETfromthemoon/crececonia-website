...
const DEFAULT_RECIPIENT = CONTACT_EMAIL;
...
export async function sendPurchaseNotification({
  ...
  if (error) throw new Error(`Resend rechazó la notificación de compra: ${error.message}`);
  if (!data?.id) throw new Error("Resend no devolvió el identificador de la notificación de compra");
  return data.id;
}
