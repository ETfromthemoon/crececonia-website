...
export class WorkshopRecoveryRecipientError extends Error {
  constructor(message: string) {
    super(message);
    this.name = "WorkshopRecoveryRecipientError";
  }
}
...
export async function sendWorkshopRecoveryEmail({
  ...
  if (error) {
    const rejectedRecipient = /invalid [`'"]?to|invalid recipient|email address is invalid/i.test(error.message);
    if (rejectedRecipient) {
      throw new WorkshopRecoveryRecipientError(error.message);
    }
    throw new Error(error.message);
  }
  if (!data?.id) throw new Error("Resend no confirmó el correo de recuperación.");
  return data.id;
}
