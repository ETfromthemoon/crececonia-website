...
export default async function WorkshopAdminPage({searchParams}:{searchParams:Promise<{key?:string}>}){
  ...
  return <main className="workshop-admin"><header><div><p>Admin · workshop en vivo</p><h1>{WORKSHOP_TITLE}</h1><span>{WORKSHOP_SESSION_LABEL}</span></div><a href="/workshop-en-vivo-2026-09-06" target="_blank">Ver página ↗</a></header><div className="workshop-admin-live"><WorkshopHealthPanel adminKey={key}/><WorkshopSalesControls adminKey={key} initial={availability}/></div><WorkshopDashboardLive adminKey={key} initial={dashboard}/><section className="workshop-admin-operation"><div><p>Operación de la sala</p><h2>Publica cada recurso cuando esté listo.</h2><span>Agrega aquí la grabación y SKOOL. Slides, hoja de trabajo y skills se suben juntos con <code>npm run workshop:subir-recursos</code>.</span></div><WorkshopSettingsForm adminKey={key} initial={settings}/></section></main>;
}
