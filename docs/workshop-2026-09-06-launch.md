...
4. Ejecutar `npm run workshop:subir-recursos`. El comando exige exactamente cinco skills, genera el ZIP, sube ZIP + slides + hoja al Storage privado y verifica los tres objetos. Si se modifican los slides, reemplazar `slides-taller-claude-desktop.html` y volver a ejecutar el comando: el objeto anterior se reemplaza. `*.original.html` y `GUIA-RELATOR.md` son internos y no se entregan. `workshop:subir-skills` se conserva como alias compatible.
...
7. Abrir `/admin/workshop-2026-09-06?key=ADMIN_SECRET`. Pegar el enlace de la grabación en **Grabación del taller** y verificar que el archivo acepte lectores externos con el enlace. Configurar el correo de soporte. SKOOL puede quedar vacío hasta su lanzamiento y no bloquea las ventas; cuando exista la invitación, guardarla en ese mismo panel y pulsar **Enviar recursos a todos** para avisar a quienes ya compraron.
