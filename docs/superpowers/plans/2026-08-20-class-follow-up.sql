-- Correo postclase idempotente. Se aplica después de class-session-reminders.sql.
...
alter table commerce.class_delivery_events
  ...
  check (delivery_kind in ('welcome', 'ebooks', 'session-1h', 'session-10m', 'session-late', 'follow-up'));
...
as $$
begin
  if p_delivery_kind not in ('welcome', 'ebooks', 'session-1h', 'session-10m', 'session-late', 'follow-up') then
